...
describe("computeAdminChecklistProgress", () => {
  it("uses fixed protocol total 19 even when rows are missing", () => {
    assert.equal(ALL_SEED_TEMPLATE_KEYS.length, ADMIN_CHECKLIST_STEP_TOTAL);

    const noneDone = computeAdminChecklistProgress([]);
    assert.equal(noneDone.total, 19);
    assert.equal(noneDone.completed, 0);
    assert.equal(noneDone.currentStepName, "Enrolment Survey");

    const level1Only = LEVEL_1_TEMPLATE_KEYS.map((key) =>
      item(key, "COMPLETED")
    );
    const klaraLike = computeAdminChecklistProgress(level1Only);
    assert.equal(klaraLike.total, 19);
    assert.equal(klaraLike.completed, 9);
    assert.equal(klaraLike.currentStepName, "3 Month Survey");
  });
